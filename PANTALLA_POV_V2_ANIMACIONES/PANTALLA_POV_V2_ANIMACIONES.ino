...
const int NUM_IMAGES = 80;  //Escribe aquí el número de imágenes que tenga tu animación
...
void setup() {
  ...
  Serial.begin(115200);
  delay(2000);
  Serial.println(pgm_read_dword(&(frames[1][2][0])));
...
void draw_leds(int leds_pos){
  ...
  for(int i=0; i<NUM_LEDS; i++){
    //color16 = pgm_read_dword(&(frame0[row][i]));
    color16 = pgm_read_dword(&(frames[cont][row][i]));
    color24 = expandColor(color16);
    leds1[i] = color24;
    
    //color16 = pgm_read_dword(&(frame0[row][NUM_LEDS + i]));
    color16 = pgm_read_dword(&(frames[cont][row][NUM_LEDS + i]));
    color24 = expandColor(color16);
    leds2[i] = color24;
  }
  ...
  for(int i=0; i<NUM_LEDS; i++){
    //color16 = pgm_read_dword(&(frame0[row][2*NUM_LEDS-i-1]));
    color16 = pgm_read_dword(&(frames[cont][row][2*NUM_LEDS-i-1]));
    color24 = expandColor(color16);
    leds3[i] = color24;

    //color16 = pgm_read_dword(&(frame0[row][NUM_LEDS-i-1]));
    color16 = pgm_read_dword(&(frames[cont][row][NUM_LEDS-i-1]));
    color24 = expandColor(color16);
    leds4[i] = color24;
  }    
}
